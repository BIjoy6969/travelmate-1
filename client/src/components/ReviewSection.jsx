...
const ReviewSection = ({ destinationId, destinationName, onClose, onReviewChange }) => {
        ...
        <div className="space-y-6">
                ...
                {!showForm && (
                    <div className="flex flex-col items-end gap-1">
                        <button
                            onClick={() => {
                                if (!USER_ID) {
                                    alert("Login necessary to write a review.");
                                    navigate('/login');
                                    return;
                                }
                                setShowForm(true);
                        ...
                        {!USER_ID && <span className="text-[10px] text-slate-400 italic">Login required to post</span>}
                    </div>
                )}
            </div>
            ...
            {showForm && (
                <div className="p-4 bg-minimal-surface rounded-xl border border-minimal-border space-y-4 animate-in-faded">
                    <div className="flex items-center justify-between">
                        <h4 className="text-sm font-bold text-minimal-text">New Public Review</h4>
                        <span className="text-[10px] text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full font-bold">VISIBLE TO ALL USERS</span>
                    </div>
