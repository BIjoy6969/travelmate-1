...
const HotelSearch = () => {
    ...
    const searchHotels = async (e) => {
        ...
        setLoading(true);
        try {
            const res = await hotelService.search({ location });
            setHotels(res.data);
        } catch (err) {
            console.error(err);
    ...
    return (
        <div>
            <div className="bg-white p-6 rounded-lg shadow-md mb-8">
                ...
                <form onSubmit={searchHotels} className="flex gap-4 items-end">
                    <div className="flex-grow">
                        <label className="block text-sm font-medium text-gray-700 mb-1">Where do you want to stay?</label>
                        <PlaceAutocomplete onPlaceSelect={(place) => setLocation(place.name)} />
                    </div>
                    <button className="bg-blue-600 text-white px-6 py-3.5 rounded-xl font-bold hover:bg-blue-700 h-[52px]">
                        Search Hotels
                    </button>
                </form>
            </div>
