...
import { tripService, exportService, DEMO_USER_ID } from "../services/api";
...
export default function Trips() {
  ...
  const loadTrips = async () => {
    ...
    setLoading(true);
    try {
      const res = await tripService.getAll(DEMO_USER_ID);
      setTrips(res.data || []);
    } catch {
      setTrips([]);
  ...
  const handleSubmit = async (e) => {
    ...
    setError("");
    try {
      await tripService.create({ ...formData, userId: DEMO_USER_ID });
      setShowForm(false);
      setFormData({
        title: "",
        destination: "",
