...
const AIChat: React.FC = () => {
    ...
    const handleSend = async (e: React.FormEvent) => {
        ...
        try {
            const res = await api.post('/chat', { userId: user._id, message: userMsg });
            setMessages(prev => [...prev, { sender: 'ai', text: res.data.reply || "Sorry, I couldn't process that." }]);
        } catch (error: any) {
            console.error("Chat Error:", error);
            setMessages(prev => [...prev, { sender: 'ai', text: "I'm having trouble connecting to the travel grid. Please try again." }]);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="min-h-screen bg-gray-100 flex flex-col">
            ...
            <div className="flex-1 max-w-4xl mx-auto w-full p-4 flex flex-col">
                ...
                <div className="flex-1 bg-white border-x border-b rounded-b-2xl p-4 overflow-y-auto space-y-4 shadow-sm" style={{ maxHeight: '600px' }}>
                    {messages.map((msg, idx) => (
                        <div key={idx} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`flex max-w-[80%] items-start gap-2 ${msg.sender === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                                ...
                                <div className={`p-3 rounded-2xl ${msg.sender === 'user' ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-gray-100 text-gray-800 rounded-tl-none'}`}>
                                    {msg.text}
                                </div>
                            </div>
                        </div>
