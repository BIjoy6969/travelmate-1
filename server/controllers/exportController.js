...
// countryCode -> currency code (simple map good for demo)
const currencyByCountry = (cc = "") => {
...
// Draw nice section header
function sectionHeader(doc, title, color = "#0ea5e9") {
  const y = doc.y;
  doc
    .save()
    .roundedRect(50, y, 495, 24, 8)
    .fill(color)
    .restore();
...
export const exportTripPDF = async (req, res) => {
  ...
  try {
    ...
    doc
      .fillColor("#ffffff")
      .fontSize(22)
      .text("TravelMate Report", 50, 28, { align: "left" });
    ...
    if (tripId) {
      ...
      doc.text(
        usedCity
          ? `Location selected: ${usedCity}`
          : "No location selected (Weather & Currency will be blank)."
      );
      doc.fillColor("#000000");
      doc.moveDown(1);
    }
    ...
    if (!usedCity) {
      doc.fillColor("#64748b").fontSize(11).text("--").fillColor("#000000");
      doc.moveDown(1);
    } else if (!weather?.ok) {
      doc.fillColor("#b00020").fontSize(11).text("Weather not available.").fillColor("#000000");
      doc.moveDown(1);
    } else {
    ...
    if (!usedCity) {
      ...
      if (!money.ok) {
        doc.fillColor("#b00020").fontSize(11).text("Currency not available.").fillColor("#000000");
        doc.moveDown(1);
      } else {
        ...
        keyValue(
          doc,
          "Converted",
          `${money.amount} ${money.base} = ${Number(money.result).toFixed(2)} ${money.target}`
        );
        doc.moveDown(1);
      }
    }
    ...
    // -----------------------------
    // Expenses section (trip only)
    // -----------------------------
    if (trip) {
      expenses = await Expense.find({ tripId: tripId.toString() });

      sectionHeader(doc, "Expenses Summary", "#f97316");

      if (!expenses.length) {
        doc.fillColor("#64748b").fontSize(11).text("No expenses found.").fillColor("#000000");
        doc.moveDown(1);
      } else {
        let total = 0;
        expenses.forEach((e, idx) => {
          ...
          doc
            .fontSize(10)
            .fillColor("#0f172a")
            .text(`${idx + 1}. ${e.category || "Other"} - BDT ${e.amount || 0}`, { continued: false });
          if (e.description) {
        ...
        doc.moveDown(0.5);
        doc.fontSize(12).fillColor("#0f172a").text(`Total Spent: BDT ${total.toFixed(2)}`);
        doc.fillColor("#000000");
        doc.moveDown(1);
      }
    }
    ...
    doc
      ...
      .fontSize(9)
      .fillColor("#64748b")
      .text("TravelMate Report", { align: "center" })
      .fillColor("#000000");
