...
exports.exportTripPDF = async (req, res) => {
  const { tripId, city, base, target = "BDT", amount = 1 } = req.query;
  ...
  try {
    ...
    if (tripId) {
      dataObj = await Trip.findOne({ _id: tripId, userId });

      if (!dataObj) {
        sectionHeader(doc, "Error", "#ef4444");
      ...
      keyValue(doc, "Title", dataObj.title);
      keyValue(doc, "Destination", dataObj.destination);

      if (dataObj.startDate && dataObj.endDate) {
        keyValue(doc, "Dates", `${new Date(dataObj.startDate).toLocaleDateString()} - ${new Date(dataObj.endDate).toLocaleDateString()}`);
      }

      doc.moveDown(1);
      usedCity = dataObj.destination || "";
    } else {
    ...
    const derivedBase = base ? String(base).toUpperCase() : "USD";
    ...
    // Expenses section (Manual legacy expenses only)
    sectionHeader(doc, "Expenses", "#f97316");

    if (dataObj) {
      const expenses = await Expense.find({ tripId: dataObj._id, userId });
      if (expenses.length) {
        let total = 0;
