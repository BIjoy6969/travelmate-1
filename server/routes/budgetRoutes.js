...
router.post('/', async (req, res) => {
    ...
    try {
        let budget = null;
        if (destinationId) {
            budget = await Budget.findOne({ userId: req.user.id, destinationId });
        }
        if (!budget) {
            budget = await Budget.findOne({ userId: req.user.id, destination });
        }

        if (budget) {
            console.log("Updating existing budget for:", destination);
            ...
            console.log("Creating new budget for:", destination);
            budget = new Budget({
                userId: req.user.id,
                destination,
                destinationId,
...
router.post('/generate', async (req, res) => {
    ...
    try {
        ...
        3. "itinerary": array of { 
            "day": number, 
            "activities": array of { "name": string, "cost": number }, 
            "estimatedCost": number 
           }
        
        CRITICAL RULES:
        - All currency amounts MUST be in **BDT (Bangladeshi Taka)**.
        - The sum of all activity costs in the itinerary MUST equal the "totalEstimatedCost".
        - The sum of all category amounts in the breakdown MUST equal the "totalEstimatedCost".
        - For each day, "estimatedCost" MUST be the sum of its activities' costs.
        
        Only return the JSON object, no other text.`;

        console.log("Generating AI Budget for:", destination);
        const result = await model.generateContent(prompt);
        const response = await result.response;
        let text = response.text();
        console.log("Raw AI Response received:", text.substring(0, 100) + "...");
